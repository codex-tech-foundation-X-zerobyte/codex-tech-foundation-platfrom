...
uses the browser's actual `SubtleCrypto` API (SHA-1/256/384/512) — nothing
sent over the network, MD5 deliberately not offered (unsupported by
SubtleCrypto, and a broken algorithm not worth hand-rolling). Base64/URL
encode-decode handle UTF-8 correctly (`TextEncoder`/`TextDecoder`, not bare
`btoa`/`atob`, which mangle non-Latin1 text). UUID generator uses
`crypto.randomUUID()`. JSON formatter validates and pretty-prints. **QR
code generation is disclosed, not hidden, as depending on a public external
service** (`api.qrserver.com`) — a real client-side QR encoder needs
Reed-Solomon error correction, genuinely non-trivial to implement correctly
from scratch, and this project has no verified QR library to build
against; faking it or half-implementing something that could silently
produce an unscannable code would be worse than an honestly-labeled
external dependency. Routed at `/worker/devtools` and `/admin/devtools`.

**Light/Dark/System theme — a real designed palette, not inverted dark
colors.** Since the whole design system already runs on CSS custom
properties, the correct approach was a complete parallel token set under
`[data-theme='light']` (plus a `prefers-color-scheme` media-query fallback
for "System"), not per-component overrides — every surface adapts
automatically because everything already references `var(--token)` rather
than hardcoded colors. **Found and fixed while auditing for this**: two
hardcoded hex colors in `Button.css` that would have broken specifically in
light mode — most notably the danger button's light-pink text, which would
have had genuinely bad contrast against a light background (a real
accessibility bug, not just a missed token). `src/lib/theme.ts` applies the
choice synchronously before React's first paint (avoiding a flash of the
wrong theme) and persists it; a real Appearance section in Account
Settings replaces having no theme control at all.

**Animated background — the visitor area, plus a restrained workspace
variant.** Soft drifting gradient blobs (pure CSS, GPU-accelerated
transforms) layered with a lightweight canvas particle-network (~46 nodes,
capped device-pixel-ratio, proximity-based connecting lines) for the public
site; the workspace variant drops the canvas layer entirely and only shows
the blobs, per the brief's own "restrained... dashboards prioritize
readability" framing — not the same effect at lower opacity. Fully
respects `prefers-reduced-motion` (no animation frame loop at all when set,
not just a slower one) and cleans up its `requestAnimationFrame`
loop/resize listener on unmount. **Bug caught before shipping**: the first
version force-set `position: relative` on every direct child of the public
layout to sit above the fixed background — but the mobile navigation
drawer overlay is also a direct child, and that blanket selector's
specificity would have beaten the drawer's own `position: fixed` rule,
silently breaking mobile navigation. Caught by checking actual CSS
specificity, not by assuming a plausible-looking selector was safe; fixed
by targeting `header`/`main`/`footer` explicitly instead of a `:not()` catch-all.

**Live-error diagnostic pass — real bugs confirmed from source, not
guessed.** A document reporting four live browser errors (profiles request
timeout, `channels` 403, `worker_profiles` 400, WebSocket handshake
timeout) also asserted the chat architecture should use
`chat_channels`/`chat_channel_members`/`chat_messages` — **that's not
this codebase's real schema** (it's `channels`/`channel_members`/`messages`,
built in Pass 5), and renaming to match an unfounded assumption would have
meant inventing a competing duplicate system rather than fixing anything —
declined, and flagged explicitly rather than silently going along with it.

What *was* real and fixed, confirmed directly against the schema:
- **The exact cause of the `worker_profiles` 400.** `countRows()` (a
  generic dashboard-count helper) was hardcoded to `select('id', ...)`,
  assuming every table has a column literally named `id`. Every table does
  except `worker_profiles`, whose primary key is `user_id` (it's a 1:1
  extension of `profiles`) — a real, isolated query bug, not a live-database
  mismatch. Now takes an `idColumn` parameter; `AdminDashboard.tsx`'s
  worker-count metric passes `user_id`.
- **`AdminDashboard.tsx` had hardcoded fake health status** — three lines
  permanently reading "Supabase connection: healthy," "Row-level security:
  enforced," "Edge functions: reachable" regardless of actual state, which
  both audit documents in this project explicitly warned against and which
  would have been actively lying to the user during the very outage being
  reported. Replaced with `healthCheck.ts`: a real timed database round
  trip and a real Edge Function reachability check (a side-effect-free CORS
  preflight OPTIONS request), each showing checking/healthy-with-latency/
  degraded/unavailable-with-error, plus a manual recheck button. The
  unverifiable "RLS: enforced" line was dropped entirely rather than kept
  as another unearned claim — it can't be meaningfully checked from an
  authenticated admin session.
- **`supabase.ts` silently fell back to a fake project** (`example.supabase.co`
  / a demo anon key) when environment variables were missing — exactly the
  kind of misconfiguration that produces a wall of confusing, seemingly-
  unrelated per-request errors (which is what the reported symptoms look
  like). Now exports `hasValidConfig`; `main.tsx` checks it before
  rendering and shows one clear "Configuration error" message instead of
  booting the app against a project that was never real.
- **The service worker had no cache-cleanup logic at all** — `activate`
  only called `self.clients.claim()`, never deleted old caches. Bumped to
  `codex-shell-v2` and added actual cleanup, so a returning visitor's
  browser can't keep serving an old cached shell indefinitely after a real
  fix ships.

**What I could NOT verify or fix, and said so rather than guessing**: the
`profiles` connection timeout, the exact PostgREST-level cause of the
`channels` 403 (as opposed to an empty result), and the WebSocket handshake
timeout are, on the evidence available, most consistent with a live
Supabase project/environment issue (wrong project reference, a paused or
deleted project, or migrations genuinely not applied to whichever project
is actually live) — not something fixable by editing repository source.
This can't be confirmed without direct access to the live Supabase project
